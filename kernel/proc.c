...
//almost same as kwait but we have rusage now(for cputime/percentage)
int
    ...
    for (pp = proc; pp < &proc[NPROC]; pp++) {
      if (pp->parent == p) {
        ...
        if (pp->state == ZOMBIE) {
          ...
          // Copy out the rusage structure
          if (rusage_addr != 0) {
            struct rusage r;
            r.cputime = pp->cputime; // Fetches cputime added in the previous step
            if (copyout(p->pagetable, p->sz, rusage_addr, (char *)&r, 
                        sizeof(r)) < 0) {
              release(&pp->lock);
              release(&wait_lock);
              return -1;
